...
def create_kpi_gauge(value: Optional[Union[int, float, np.number]], title_key: str, lang: str,
    ...
    if max_value_override is not None and pd.notna(max_value_override):
        ...
        if max_axis_val <= 0 : max_axis_val = 10.0
    steps_list = []
...
# --- Comparison Bar Chart Visualization (Corrected for hovertemplate) ---
def create_comparison_bar_chart(df_input: pd.DataFrame, x_col: str,
                                y_cols_map: Dict[str, str], # {display_key_for_legend: actual_col_name_in_df}
    ...
    # Prepare DataFrame for Plotly Express by renaming columns for the legend
    # The `y` argument in px.bar will take the list of these NEW display names.
    df_plot = df[[x_col]].copy() # Start with x_col, will add renamed y_cols
    y_display_names_for_px = [] # These will be the names used in px.bar 'y' list
    original_y_cols_for_sum = [] # Store original column names for sum if stacked
    ...
    for display_key, actual_col_name in y_cols_map.items():
        if actual_col_name in df.columns and pd.api.types.is_numeric_dtype(df[actual_col_name]):
            ...
            df_plot[display_name] = df[actual_col_name] # Create new column with display name, copy data
            y_display_names_for_px.append(display_name)
            original_y_cols_for_sum.append(actual_col_name) # Keep track of original columns used
        # else:
            # Consider logging or st.warning if a column in y_cols_map is not found or not numeric in df

    if df_plot.empty or x_col not in df_plot.columns or not y_display_names_for_px:
        return go.Figure().update_layout(
            title_text=f"{title_text} ({get_lang_text(lang, 'no_data_for_selection')})",
            annotations=[dict(text=get_lang_text(lang, 'no_data_for_selection'), showarrow=False, xref="paper", yref="paper", x=0.5, y=0.5, font_size=12)]
        )
    
    # data_label_format_str is just the format specifier part (e.g., ".0f")
    # Plotly templates use %{variable:formatspecifier}
    fmt_spec = data_label_format_str if (isinstance(data_label_format_str, str) and data_label_format_str) else ".0f"
    
    texttemplate_final = f'%{{y:{fmt_spec}}}'
    hovertemplate_final = f'<b>%{{x}}</b><br>%{{fullData.name}}: %{{y:{fmt_spec}}}<extra></extra>'
    
    fig = px.bar(df_plot, x=x_col, y=y_display_names_for_px, # Use the renamed columns
                 title=None, # Title is set in update_layout for better control
                 barmode=barmode,
                 color_discrete_sequence=config.COLOR_SCHEME_CATEGORICAL,
                 # `labels` in px.bar maps original column names (from `y=`) to new axis/legend titles
                 # Since we used display names in `y=`, px.bar should use these for legend by default.
                 # If further refinement of legend names is needed beyond `display_name`, use labels here.
                 # labels={disp_name: disp_name for disp_name in y_display_names_for_px} # Essentially no change here
                )
    ...
    fig.update_traces(
        ...
        selector=dict(type='bar') # Apply to all bar traces created by px.bar
    )

    if barmode == 'stack' and show_total_for_stacked and original_y_cols_for_sum:
        # Sum the original data columns from the input 'df' for accuracy before any renaming for display
        # or ensure df_plot contains the correct values under y_display_names_for_px
        if all(col_name in df_plot.columns for col_name in y_display_names_for_px): # Ensure columns exist in df_plot
            df_plot['_total_stacked_val_'] = df_plot[y_display_names_for_px].sum(axis=1)
            annotations_list = [
                dict(x=row[x_col], y=row['_total_stacked_val_'], 
                     text=f"{row['_total_stacked_val_']:{fmt_spec}}", # Apply format
                     font=dict(size=10, color=config.COLOR_TEXT_SECONDARY),
                     showarrow=False, yanchor='bottom', yshift=3, xanchor='center')
                for _, row in df_plot.iterrows() if pd.notna(row['_total_stacked_val_'])
            ]
            if annotations_list: 
    ...
    fig.update_layout(
        ...
        yaxis_title=y_title_text, xaxis_title=x_title_text,
        legend_title_text=get_lang_text(lang, "metrics_legend"), hovermode="x unified",
        hoverlabel=dict(bgcolor="rgba(255,255,255,0.9)", font_size=11, namelength=-1), # Ensure full names
...
def display_metric_card(st_object, label_key: str, value: Optional[Union[int, float, np.number]], lang: str,
                        ...
                        higher_is_better: Optional[bool] = None, help_text_key: Optional[str] = None, # Key for help text
                        target_value: Optional[Union[int, float, np.number]] = None,
                        threshold_good: Optional[Union[int, float, np.number]] = None,
                        threshold_warning: Optional[Union[int, float, np.number]] = None):
    label_text_orig = get_lang_text(lang, label_key)
    
    # Format help text if placeholders are used
    ...
    if target_value is not None and pd.notna(target_value) and isinstance(target_value, (int, float, np.number)) and "{target}" in raw_help_text_template:
        ...
        target_format_spec = ".0f" if target_float % 1 == 0 else ".1f"
        try:
            help_text_final_str = raw_help_text_template.format(target=f"{target_float:{target_format_spec}}")
        except KeyError: # In case there are other {} placeholders not meant for target
            help_text_final_str = raw_help_text_template # Fallback to unformatted
    ...
    if pd.notna(value) and isinstance(value, (int, float, np.number)):
        ...
        elif unit == get_lang_text(lang, 'days_unit') or (abs(val_raw_float) >= 1000 and val_raw_float % 1 == 0):
        ...
        if previous_value is not None and pd.notna(previous_value) and isinstance(previous_value, (int, float, np.number)):
            ...
            delta_unit_for_display = unit if unit != '%' else '' # For the absolute part of delta
...
def create_enhanced_radar_chart(df_radar_input: pd.DataFrame, categories_col: str, values_col: str,
    ...
    all_r_values_list = df_radar[values_col].dropna().tolist() # Ensure no NaNs from data for max calculation
    if target_values_map: 
        all_r_values_list.extend([v for v in target_values_map.values() if pd.notna(v) and isinstance(v,(int,float))])
    
    valid_r_values_list = [float(v) for v in all_r_values_list if isinstance(v, (int,float)) and pd.notna(v)]
    max_data_val_for_range = max(valid_r_values_list) if valid_r_values_list else 0.0 # Default to 0 if no valid values
...
def create_stress_semaforo_visual(stress_level_value: Optional[Union[int, float, np.number]], lang: str,
    ...
    fig = go.Figure(go.Indicator(
        mode="gauge+number", value=val_for_bar_indicator,
