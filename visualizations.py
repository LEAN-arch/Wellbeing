...
def get_status_by_thresholds(value, higher_is_worse, threshold_good=None, threshold_warning=None):
    ...
    if higher_is_worse:
        ...
        if warn_thresh is not None and val_float <= warn_thresh:
            if good_thresh is None or val_float > good_thresh:
                return "warning"
...
def create_kpi_gauge(value, title_key, lang, unit="%", higher_is_worse=True,
    ...
    if max_value_override is not None:
        ...
        if valid_thresh_for_max: max_val_candidates.append(max(float(t) for t in valid_thresh_for_max) * 1.2)
        else:
            if not pd.notna(current_value_for_gauge) or current_value_for_gauge == 0: max_val_candidates.append(100.0)
    ...
    if higher_is_worse:
        ...
        if num_t_warn is not None:
            ...
            current_range_start = num_t_warn
        if num_t_good is not None and num_t_good > current_range_start:
...
def create_trend_chart(df_input, date_col, value_cols_map, title_key, lang,
                       y_axis_title_key="value_axis_label", x_axis_title_key="date_time_axis_label",
                       show_average_line=False, target_value_map=None,
                       rolling_avg_window=None, value_col_units_map=None):
    df = df_input.copy()
    ...
    processed_value_cols_actual_names = []
    ...
    if rolling_avg_window and isinstance(rolling_avg_window, int) and rolling_avg_window > 1:
        for i, actual_col_name in enumerate(processed_value_cols_actual_names):
            series_color = colors[i % len(colors)]
            series_label_key_for_rolling = next((k for k, v in value_cols_map.items() if v == actual_col_name), actual_col_name)
            base_series_name = get_lang_text(lang, series_label_key_for_rolling, actual_col_name.replace('_', ' ').title())
            rolling_avg_name = f"{base_series_name} ({rolling_avg_window}-p MA)"
            unit_suffix = value_col_units_map.get(actual_col_name, "") if value_col_units_map else ""
            if len(df) >= rolling_avg_window:
                df[f'{actual_col_name}_rolling_avg'] = df[actual_col_name].rolling(window=rolling_avg_window, min_periods=1, center=True).mean()
                fig.add_trace(go.Scatter(
                    x=df[date_col], y=df[f'{actual_col_name}_rolling_avg'], mode='lines',
                    name=rolling_avg_name, line=dict(color=series_color, width=1.5, dash='dot'), opacity=0.7,
                    hovertemplate=f"<b>{rolling_avg_name}</b><br>{x_title_text}: %{{x|%Y-%m-%d}}<br>{y_title_text}: %{{y:.2f}}{unit_suffix}<extra></extra>"
                ))
    for i, actual_col_name in enumerate(processed_value_cols_actual_names):
        series_color = colors[i % len(colors)]
        series_label_key_for_static = next((k for k,v in value_cols_map.items() if v == actual_col_name), actual_col_name)
        series_name = get_lang_text(lang, series_label_key_for_static, actual_col_name.replace('_', ' ').title())
        ...
        if target_value_map and actual_col_name in target_value_map and pd.notna(target_value_map[actual_col_name]):
            ...
            fig.add_hline(y=target_val, line_dash="dash",
                          ...
                          annotation_bgcolor="rgba(255,255,255,0.75)")
    fig.update_layout(
        title=dict(text=title_text, x=0.5), yaxis_title=y_title_text, xaxis_title=x_title_text,
        legend_title_text=get_lang_text(lang, "metrics_legend", "Metrics"), hovermode="x unified",
        hoverlabel=dict(bgcolor="white", font_size=12, bordercolor=config.COLOR_GRAY_TEXT, namelength=-1),
        xaxis=dict(showgrid=False, type='date', rangeslider_visible=True,
            rangeselector=dict(buttons=list([
                    ...
                    dict(step="all")]), font=dict(size=10))),
...
def create_comparison_bar_chart(df_input, x_col, y_cols_map, title_key, lang,
    ...
    df_for_plot = df.copy()
    y_display_names_for_px = [] # This list will contain the names px.bar will use (localized)
    # This map helps to select original data for aggregation but use display names for plotting
    ...
    for series_key, actual_col_name in y_cols_map.items():
        if actual_col_name in df.columns and pd.api.types.is_numeric_dtype(df[actual_col_name]):
            ...
            # If display_name is different from actual_col_name, we'll need to rename
            if display_name != actual_col_name:
                actual_to_display_map[actual_col_name] = display_name
            y_display_names_for_px.append(display_name) # Use the name that will be in df_for_plot
    
    df_for_plot.rename(columns=actual_to_display_map, inplace=True)

    # Ensure all y_display_names_for_px exist in df_for_plot and are numeric
    y_display_names_final = [name for name in y_display_names_for_px if name in df_for_plot.columns and pd.api.types.is_numeric_dtype(df_for_plot[name])]


    if df_for_plot.empty or x_col not in df_for_plot.columns or not y_display_names_final:
        return go.Figure().update_layout(
            title_text=f"{title_text} ({lang_texts.get('no_data_for_selection', 'No data')})",
            annotations=[dict(text=lang_texts.get('no_data_for_selection', 'No data'), showarrow=False, xref="paper", yref="paper", x=0.5, y=0.5, font_size=12)]
        )
    
    fig = px.bar(df_for_plot, x=x_col, y=y_display_names_final, title=None, barmode=barmode,
                 color_discrete_sequence=config.COLOR_SCHEME_CATEGORICAL,
                 # labels will be automatically generated from y_display_names_final
                 )

    safe_format_spec = data_label_format_str if isinstance(data_label_format_str, str) and data_label_format_str else ".0f"

    fig.update_traces(
        texttemplate=f'%{{y:{safe_format_spec}}}',
        textposition='outside' if barmode != 'stack' else 'inside',
        textfont_size=9,
        insidetextanchor='middle' if barmode == 'stack' else 'auto',
        hovertemplate=f'<b>%{{x}}</b><br>%{{fullData.name}}: %{{y:{safe_format_spec}}}<extra></extra>', # %{fullData.name} is legend entry
        selector=dict(type='bar')
    )

    if barmode == 'stack' and show_total_for_stacked and y_display_names_final:
        df_for_plot['_total_stacked_'] = df_for_plot[y_display_names_final].sum(axis=1)
        annotations = []
        for _, row in df_for_plot.iterrows():
            ...
            if pd.notna(total_val):
                 annotations.append(dict(
                                    x=x_val, y=total_val,
                                    text=f"{total_val:{safe_format_spec}}", # Apply format
    ...
    fig.update_layout(
        ...
        xaxis_tickangle=-30 if not df_for_plot.empty and len(df_for_plot[x_col].unique()) > 6 else 0,
...
def display_metric_card(st_object, label_key, value, lang,
    ...
    if pd.notna(value) and isinstance(value, (int, float, np.number)):
        ...
        if previous_value is not None and pd.notna(previous_value) and isinstance(previous_value, (int,float, np.number)):
            ...
            else: formatted_delta_abs = f"{abs(delta_abs_val):.1f}{(' ' + delta_display_unit) if delta_display_unit else ''}"
...
def create_enhanced_radar_chart(df_radar_input, categories_col, values_col, title_key, lang,
    ...
    if target_values_map: all_r_values_for_range.extend([v for v in target_values_map.values() if pd.notna(v) and isinstance(v, (int,float))])
    ...
    if target_values_map:
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            line=dict(color=config.COLOR_TARGET_LINE, dash='dash', width=2), hoverinfo='skip'
        ))
    fig.update_layout(
        title=dict(text=title_text, x=0.5, font=dict(size=18)),
        polar=dict(bgcolor="rgba(255,255,255,0.05)",
            radialaxis=dict(visible=True, range=[0, range_max], showline=False, showticklabels=True,
                            gridcolor="rgba(0,0,0,0.15)", linecolor="rgba(0,0,0,0.15)", tickfont=dict(size=9)),
            angularaxis=dict(showline=False, showticklabels=True, gridcolor="rgba(0,0,0,0.15)",
                             linecolor="rgba(0,0,0,0.15)", tickfont=dict(size=10), direction="clockwise")),
        showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.05, xanchor="center", x=0.5, font=dict(size=9)),
...
def create_stress_semaforo_visual(stress_level, lang, scale_max=config.STRESS_LEVEL_MAX_SCALE):
    ...
    number_config = {
        ...
        'valueformat': ".1f" # Format for the displayed number
    }
    if display_number_indicator is not None: # Only add suffix if there's a number to show
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            # The bar value for positioning. Let Indicator's 'value' prop control this internally based on what we feed it.
            # The 'bar.color' is more for visual cue matching status if steps aren't enough.
            'bar': {'color': semaforo_color if pd.notna(display_number_indicator) else config.COLOR_GRAY_TEXT, 'thickness': 0.6},
