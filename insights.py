...
def _get_insight_text(lang_code: str, key: str, default_text: Optional[str] = None, **kwargs) -> str:
    ...
    for k, v in kwargs.items():
        if isinstance(v, (float, int, np.number)) and pd.notna(v): # np.number is now defined
            # Check if key implies percentage to format it nicely for insights
            # Using simple string checks; more robust might involve passing format hints
            if "thresh" in k or "val" in k or "target" in k or "change" in k or "days" in k or "count" in k :
                # Attempt to format as float with 1 decimal, or int if it's a whole number
                try:
                    float_v = float(v)
                    if float_v % 1 == 0 and abs(float_v) < 10000 : # Integers (not too large)
                    ...
                    else: # default to one decimal
                        formatted_kwargs[k] = f"{float_v:.1f}"
                except (TypeError, ValueError):
                    formatted_kwargs[k] = str(v) # fallback to string if float conversion fails
            else:
                formatted_kwargs[k] = v # Pass as is for other types or non-numeric identified keys
        elif v is None or pd.isna(v):
            # Get localized "N/A" or a default if that key is missing
            na_text_key = "status_na_label" # Make sure this key exists in TEXT_STRINGS
            formatted_kwargs[k] = config.TEXT_STRINGS.get(lang_code, config.TEXT_STRINGS[config.DEFAULT_LANG]).get(na_text_key, "N/A")
        else:
    ...
    try:
        ...
        # print(f"Insight text formatting WARNING (KeyError): For lang '{lang_code}', key '{key}', placeholder '{e}' missing. Kwargs: {kwargs}, Formatted Kwargs: {formatted_kwargs}, Base Text: '{base_text}'")
        return base_text
    except TypeError as e:
        # print(f"Insight text formatting WARNING (TypeError): For lang '{lang_code}', key '{key}'. Error: {e}. Base: '{base_text}'. Formatted Kwargs: {formatted_kwargs}")
        return base_text
    except ValueError as e:
        # print(f"Insight text formatting WARNING (ValueError): For lang '{lang_code}', key '{key}'. Error: {e}. Base: '{base_text}'. Formatted Kwargs: {formatted_kwargs}")
        return base_text
...
def generate_stability_insights(df_stability_filtered: pd.DataFrame, avg_rotation_val: Optional[float],
    ...
    if current_avg_rotation is not None:
        if current_avg_rotation > rot_warn_thresh:
            insights_list.append(f"🚨 **{_('rotation_high_alert')}:** " +
                                 _('rotation_high_insight_v2', # Key from config.py
                                   rotation_val=current_avg_rotation, warn_thresh=rot_warn_thresh, target_thresh=rot_target_val))
        elif current_avg_rotation > rot_good_thresh: # Using good_thresh now for moderate warning logic
            insights_list.append(f"⚠️ **{_('rotation_moderate_warn')}:** " +
                                 _('rotation_moderate_insight_v2', # Key from config.py
            ...
            insights_list.append(f"✅ **{_('rotation_good_status')}:** " +
                                 _('rotation_good_insight_v2', # Key from config.py
                                   rotation_val=current_avg_rotation, good_thresh=rot_good_thresh, target_thresh=rot_target_val)) # Added target_thresh here
    else:
        insights_list.append(_('rotation_no_data')) # Key from config.py
    ...
    if ret12_col_actual and ret12_col_actual in df_stability_filtered.columns:
        ...
        if current_avg_ret12 is not None:
            ...
            if current_avg_ret12 < ret_warn_thresh:
                insights_list.append(f"📉 **{_('retention_low_alert')}:** " +
                                     _('retention_low_insight_v2', # Key from config.py
                                       retention_val=current_avg_ret12, warn_thresh=ret_warn_thresh, target_thresh=ret_good_thresh))
            elif current_avg_ret12 < ret_good_thresh:
                insights_list.append(f"📈 **{_('retention_needs_improvement')}:** " +
                                     _('retention_improvement_insight_v2', # Key from config.py
                                       retention_val=current_avg_ret12, target_thresh=ret_good_thresh))
            else:
                insights_list.append(f"👍 **{_('retention_good_status', default='Good 12m Retention')}:** " + # Added default
                                      _('retention_good_insight_detail', retention_val=current_avg_ret12, target_thresh=ret_good_thresh)) # Key from config.py

    if hires_exits_trend_df is not None and not hires_exits_trend_df.empty and \
       'Hires_Total_Agg' in hires_exits_trend_df.columns and 'Exits_Total_Agg' in hires_exits_trend_df.columns: # Using new agg names
    ...
    # Add a generic message if no specific insights were triggered for stability
    if not insights_list and avg_rotation_val is not None : # Only if we had some data to evaluate
         insights_list.append(_('no_insights_generated_stability', default_text=_('no_insights_generated'))) # Make this specific key
...
def generate_engagement_insights(avg_enps_val_in: Optional[float], avg_climate_val_in: Optional[float],
    ...
    elif not insights_list: # If all inputs were None/NaN
           ...
           pd.api.types.is_numeric_dtype(df_stress_trend_data[psych_signals_agg_col]):
            ...
            if len(trend_data_for_diff) == 2:
                ...
                if pd.notna(workload_diff) and pd.notna(psych_diff):
                    if workload_diff > 0.2 and psych_diff < -0.2:
                        insights_list.append(f"📈 **{_('stress_trend_warn')}:** " + _('stress_trend_insight'))
    
    if not insights_list and current_avg_stress is not None : # Provide general if specific insights weren't generated but data was there
