...
# ... (Page Config, Language Selector Stub, Localization Helper, Navigation Stub - all same as last complete version) ...
...
st.set_page_config(
    ...
    page_icon=config.APP_ICON, layout="wide"
)
def display_language_selector_stub(st_session_state: Any, get_localized_text_func: Callable[[str, Optional[str]], str]) -> str:
    ...
    if 'selected_lang_code' not in st_session_state: st_session_state.selected_lang_code = config.DEFAULT_LANG
    def update_lang_callback(): st_session_state.selected_lang_code = st_session_state._app_lang_selector_key_widget_stub
    selected_lang = st.sidebar.selectbox(
        label=f"{config.TEXT_STRINGS['EN'].get('language_selector', 'Language')} / {config.TEXT_STRINGS['ES'].get('language_selector', 'Idioma')}",
        options=available_langs, index=available_langs.index(st_session_state.selected_lang_code),
        format_func=lambda x: config.TEXT_STRINGS[x].get(f"language_name_full_{x.upper()}", x.upper()),
        key="_app_lang_selector_key_widget_stub", on_change=update_lang_callback
    )
...
def display_navigation_stub() -> str:
    ...
    selected_mode = st.sidebar.radio(label=_("navigation_label", "Navigation"), options=[dashboard_nav_label, glossary_nav_label], key="app_navigation_mode_radio_stub")
    st.sidebar.markdown("---")
...
if app_mode_selected == _("dashboard_nav_label", "Dashboard"):
    ...
    # --- Sidebar Filters (stub remains the same) ---
    ...
    # ... (subtitle, alignment_note, psych_safety_note remain the same) ...
    ...
    def get_dummy_prev_val(# ... unchanged
        ...
        if variation_abs is not None or (abs(current_float_val) < 10 and not is_percent) :
            ...
            change_amount = current_float_val * factor * np.random.uniform(-0.7, 0.7)
    ...
    # --- 1. Laboral Stability Panel (remains the same) ---
    st.header(_("stability_panel_title"))
    # ... (full content of stability panel as in previous version) ...
    ...
    if not df_stability_filtered.empty:
        ...
        for i, (col_map_key_retention, label_key_retention) in enumerate(retention_metric_definitions):
            ...
            if actual_col_name_retention in df_stability_filtered.columns: value_retention = df_stability_filtered[actual_col_name_retention].mean()
        ...
        if all(col_name in df_stability_filtered.columns for col_name in [date_actual_col_stability, hires_actual_col_stability, exits_actual_col_stability]):
            ...
            if not trend_df_for_stability.empty:
                agg_trend_stability = trend_df_for_stability.groupby(pd.Grouper(key=date_actual_col_stability, freq='M')).agg(
                    Hires_Total_Agg=(hires_actual_col_stability, 'sum'), Exits_Total_Agg=(exits_actual_col_stability, 'sum')
                ).reset_index()
                ...
                st.plotly_chart(viz.create_trend_chart(
                    ...
                    "hires_vs_exits_chart_title", selected_lang_code, y_axis_title_key="people_count_label", x_axis_title_key="month_axis_label",
        ...
        action_insights_stability = insights.generate_stability_insights(
            df_stability_filtered, avg_rotation_current, agg_trend_stability if not agg_trend_stability.empty else pd.DataFrame(), selected_lang_code)
        if action_insights_stability:
            st.markdown("---"); st.subheader(_("actionable_insights_title"))
            for insight_item_stability in action_insights_stability: st.markdown(f"💡 {insight_item_stability}")
    else: st.info(_("no_data_available"))
    st.markdown("---")

    # --- 2. Safety Pulse Module (remains the same) ---
    st.header(_("safety_pulse_title"))
    # ... (full content of safety panel as in previous version) ...
    ...
    if not df_safety_filtered.empty:
        ...
        with cols_layout_safety_main[0]:
            if all(c in df_safety_filtered.columns for c in [month_col_name_safety, incidents_col_name_safety, near_misses_col_name_safety]):
                summary_safety_df = df_safety_filtered.groupby(month_col_name_safety, as_index=False).agg(
                    Incidents_Sum_Agg=(incidents_col_name_safety, 'sum'), Near_Misses_Sum_Agg=(near_misses_col_name_safety, 'sum')).reset_index()
                try:
                    ...
                    summary_safety_df.sort_values(month_col_name_safety, inplace=True); summary_safety_df.dropna(subset=[month_col_name_safety], inplace=True)
                ...
                if not summary_safety_df.empty:
                    ...
                    st.plotly_chart(viz.create_comparison_bar_chart(
                        ...
                        x_axis_title_key="month_axis_label", y_axis_title_key="count_label", barmode='stack', show_total_for_stacked=True, data_label_format_str=".0f"
                    ), use_container_width=True)
        ...
        if days_no_acc_col_name_safety in df_safety_filtered.columns: current_dwa_val = df_safety_filtered[days_no_acc_col_name_safety].max()
        with cols_layout_safety_main[1]:
            previous_dwa_value = get_dummy_prev_val(current_dwa_val, 0.1, variation_abs=10)
            viz.display_metric_card(st, "days_without_accidents_metric", current_dwa_val, selected_lang_code, unit=" "+_("days_unit"), higher_is_better=True, help_text_key="days_no_incident_help",
                                   threshold_good=config.SAFETY_DAYS_NO_INCIDENTS["good"], threshold_warning=config.SAFETY_DAYS_NO_INCIDENTS["warning"], previous_value=previous_dwa_value)
        with cols_layout_safety_main[2]:
            active_alerts_current_val = float('nan')
            if active_alerts_col_name_safety in df_safety_filtered.columns: active_alerts_current_val = df_safety_filtered[active_alerts_col_name_safety].sum()
            ...
            viz.display_metric_card(st, "active_safety_alerts_metric", active_alerts_current_val, selected_lang_code, unit="", higher_is_better=False, target_value=0,
                                   threshold_good=0, threshold_warning=1, previous_value=previous_active_alerts_val)
    ...
    # --- 3. Employee Engagement & Commitment (remains the same) ---
    st.header(_("engagement_title"))
    # ... (full content of engagement panel as in previous version) ...
    ...
    # --- 4. Operational Stress Dashboard (remains the same) ---
    st.header(_("stress_title"))
    # ... (full content of stress panel as in previous version) ...
    ...
    # --- 5. Spatial Dynamics Analysis (UPDATED TO BE FUNCTIONAL) ---
    st.header(_("plant_map_title")) # Main title for the section

    # Determine which DataFrame to use for spatial analysis (e.g., stress or safety)
    # For this example, we'll primarily use df_stress_filtered.
    # You might want to add a selector to choose which metric to map (stress, incidents, etc.)
    
    # Define which metric to use as the 'z' value for the metric density heatmap
    metric_to_map_col_key = "stress_level_survey" # Example: can be changed to "incidents"
    metric_to_map_actual_col = config.COLUMN_MAP.get(metric_to_map_col_key)
    
    # Select a base DataFrame that contains the metric and potentially coordinates
    # df_for_spatial_analysis = df_stress_filtered if metric_to_map_col_key == "stress_level_survey" else df_safety_filtered
    # More robustly, choose the one that actually has the metric
    if metric_to_map_actual_col in df_stress_filtered.columns:
        df_for_spatial_analysis = df_stress_filtered
        colorbar_title_key_for_metric_map = "stress_level_label_short"
    elif metric_to_map_actual_col in df_safety_filtered.columns:
        df_for_spatial_analysis = df_safety_filtered
        colorbar_title_key_for_metric_map = "incident_count_label_short" # Make sure this key exists for incident context
    else:
        df_for_spatial_analysis = pd.DataFrame() # Empty if metric column not found
        colorbar_title_key_for_metric_map = "value_axis_label" # Generic fallback


    if not df_for_spatial_analysis.empty:
        x_coord_col_key = "location_x"
        y_coord_col_key = "location_y"
        actual_x_coord_col = config.COLUMN_MAP.get(x_coord_col_key)
        actual_y_coord_col = config.COLUMN_MAP.get(y_coord_col_key)

        spatial_df_prepared = df_for_spatial_analysis.copy()

        # --- Coordinate Handling: Use real if present, else simulate ---
        has_real_coords = (actual_x_coord_col and actual_x_coord_col in spatial_df_prepared.columns and
                           ...
                           spatial_df_prepared[[actual_x_coord_col, actual_y_coord_col]].notna().all(axis=1).any())

        x_col_for_map_final = None
        y_col_for_map_final = None

        if has_real_coords:
            x_col_for_map_final = actual_x_coord_col
            y_col_for_map_final = actual_y_coord_col
            ...
            x_col_for_map_final = "_sim_x"
            y_col_for_map_final = "_sim_y"
            np.random.seed(42)
            
            sim_base_col_key = None
            ...
            if cat_col_for_sim:
                ...
                spatial_df_prepared[x_col_for_map_final] = spatial_df_prepared[cat_col_for_sim].map(cat_to_x) + np.random.normal(0, 15, size=len(spatial_df_prepared))
                spatial_df_prepared[y_col_for_map_final] = spatial_df_prepared[cat_col_for_sim].map(cat_to_y) + np.random.normal(0, 15, size=len(spatial_df_prepared))
            else:
        ...
        # Define facility dimensions for boundary (can be dynamic or from config)
        facility_dimensions_map = None
        if not spatial_df_prepared.empty and x_col_for_map_final in spatial_df_prepared and y_col_for_map_final in spatial_df_prepared:
             facility_dimensions_map = {
                 "x0": spatial_df_prepared[x_col_for_map_final].min() - 5, # Add padding
                 "y0": spatial_df_prepared[y_col_for_map_final].min() - 5,
                 "x1": spatial_df_prepared[x_col_for_map_final].max() + 5,
                 "y1": spatial_df_prepared[y_col_for_map_final].max() + 5,
                 "width": (spatial_df_prepared[x_col_for_map_final].max() - spatial_df_prepared[x_col_for_map_final].min()) + 10, # For reference
                 "height": (spatial_df_prepared[y_col_for_map_final].max() - spatial_df_prepared[y_col_for_map_final].min()) + 10 # For reference
             }

        # Use EXAMPLE_ENTRY_EXIT_POINTS from config for demonstration
        # In a real app, these might be filtered or come from a separate data source
        entry_exit_points_to_plot = config.EXAMPLE_ENTRY_EXIT_POINTS

        if not spatial_df_prepared.empty:
            # Worker Density Heatmap
            st.subheader(_("Worker Density Heatmap", default_text_override="Worker Concentration")) # New Key for clarity
            density_map_fig = viz.create_worker_density_heatmap(
                df_input=spatial_df_prepared,
                x_col=x_col_for_map_final,
                y_col=y_col_for_map_final,
                title_key="facility_heatmap_title", # Or a more specific key
                lang=selected_lang_code,
                facility_dimensions=facility_dimensions_map,
                entry_exit_points=entry_exit_points_to_plot
            )
            st.plotly_chart(density_map_fig, use_container_width=True)
            st.markdown("---")

            # Metric Density Heatmap (e.g., Stress levels)
            # Check if metric_to_map_actual_col is valid in the prepared df
            if metric_to_map_actual_col in spatial_df_prepared.columns and not spatial_df_prepared[metric_to_map_actual_col].dropna().empty:
                metric_map_title_key = "facility_incident_heatmap_title" if metric_to_map_col_key == "incidents" else "facility_heatmap_title" # Example specific titles
                st.subheader(_(metric_map_title_key, default_text_override="Metric Intensity Map"))

                metric_heatmap_fig = viz.create_metric_density_heatmap(
                    df_input=spatial_df_prepared,
                    x_col=x_col_for_map_final,
                    y_col=y_col_for_map_final,
                    z_col=metric_to_map_actual_col,
                    title_key=metric_map_title_key,
                    lang=selected_lang_code,
                    aggregation_func="avg" if metric_to_map_col_key == "stress_level_survey" else "sum", # Sum incidents, avg stress
                    colorscale="Reds" if metric_to_map_col_key == "stress_level_survey" else "YlOrRd",
                ...
                st.warning(_("heatmap_no_value_data") + f" (Metric: {metric_to_map_col_key})")
    ...
    # ... (AI Insights panel remains the same) ...
...
# --- Optional Modules & Footer (Stubs remain the same) ---
